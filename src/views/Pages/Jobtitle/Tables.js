...
class Tables extends Component {
  ...
  handleDivision = selectedivision => {
    if(selectedivision.value !== null){
     this.setState({ selectedivision : selectedivision, selectedDivision : selectedivision.value});
    }
...
handleDate =(e,v)=>{
  ...
  handleGetDivision = () => {
    ...
    axios({
      method: 'get',
      url: urlDivision + '?page=1&size=25',
      headers: Header,
    })
      .then(data => {
        console.log(data.data.data)
    ...
    const renderresults = currentresults.map((results, index) => {
      ...
      return (
        <tr key={results.Id} data-category={results.Id}>
          ...
          <td>{results.Code}</td>
          <td>{results.Name}</td>
          <td>{results.Division.Code}</td>
          <td>{results.Division.Name}</td>
         {/* <td>
         {/* <td>
          <Button
              ...
              <Col xs="12" lg="12">
                <Card>
                  <CardHeader>
                    <Row>
                      <Col>
                        <i className="fa fa-user" /> <b>&nbsp;List Job Title</b>
                        <Button
                          style={{ marginLeft: 10 }}
                          color="success"
                          className="px-4"
